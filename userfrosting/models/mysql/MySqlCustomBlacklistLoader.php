...
class MySqlCustomBlacklistLoader extends MySqlObjectLoader {
    ...
    public static function fetch($value, $name = "id") {
        ...
        if ($results)
            return new MySqlCustomBlacklist($results, $results['id']);
        else
            return false;
    }
    ...
    public static function fetchAll($value = null, $name = null) {
        ...
        foreach ($resultArr as $id => $group)
            $results[$id] = new MySqlCustomBlacklist($group, $id);
    ...
    public static function fetchWithUsernames() {
        ...
        $bl_table = static::$tables['customblacklist']->name;
        ...
        while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            $id = $row['id'];
            $results[$id] = new MySqlCustomBlacklist($row, $row['id']);
        }
        return $results;
    }
