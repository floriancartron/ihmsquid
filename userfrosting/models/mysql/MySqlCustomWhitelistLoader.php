...
class MySqlCustomWhitelistLoader extends MySqlObjectLoader {
    ...
    public static function fetch($value, $name = "id") {
        ...
        if ($results)
            return new MySqlCustomWhitelist($results, $results['id']);
        else
            return false;
    }
    ...
    public static function fetchAll($value = null, $name = null) {
        ...
        foreach ($resultArr as $id => $group)
            $results[$id] = new MySqlCustomWhitelist($group, $id);
    ...
    public static function fetchWithUsernames() {
        ...
        $bl_table = static::$tables['customwhitelist']->name;
        ...
        while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            $id = $row['id'];
            $results[$id] = new MySqlCustomWhitelist($row, $row['id']);
        }
        return $results;
    }
